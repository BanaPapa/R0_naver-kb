...
// 주택구분(국민주택/민영주택) — 청약홈 사이트 자체 조회 폼의 houseDetailSecd(01=민영,
// 03=국민)와 동일한 개념을 odcloud 응답에서 판별한다. odcloud 서비스별로 필드명이
// HOUSE_SECD_NM / HOUSE_DTL_SECD_NM 등으로 다를 수 있어 후보를 모두 방어적으로 확인.
function houseSecdText(r) {
  return String(r.HOUSE_SECD_NM || r.HOUSE_DTL_SECD_NM || r.HOUSE_SECD || r.HOUSE_DTL_SECD || '');
}
function isNationalHousing(r) {
  return houseSecdText(r).includes('국민');
}
...
export async function fetchSupplyMonth({ yyyymm, region, keyword }) {
  ...
  // 청약홈 사이트가 houseDetailSecd=01(민영)로 조회하는 것과 동일하게 국민주택 제외.
  // 필드명 후보가 전부 비어있으면(=API 필드명이 예상과 다름) 필터가 조용히 무력화되지
  // 않도록 경고를 남긴다 — 이 경고가 뜨면 실제 응답 필드명을 확인해 위 후보에 추가할 것.
  if (rows.length && !rows.some((r) => houseSecdText(r))) {
    logger.warn(`odcloud ${yyyymm}: 주택구분 필드(HOUSE_SECD_NM 등)를 찾지 못해 국민주택 필터 비활성 상태 — 응답 필드명 확인 필요`);
  }
  const filtered = rows.filter((r) => !isNationalHousing(r));
  if (rows.length !== filtered.length) {
    logger.info(`odcloud ${yyyymm}: 국민주택 ${rows.length - filtered.length}건 제외 (민영 ${filtered.length}건 유지)`);
  }
  return filtered.map(mapSupplyRow);
}
